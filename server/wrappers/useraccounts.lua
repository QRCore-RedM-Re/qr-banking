        ...
        if not self.cardActive then
            ...
            MySQL.Async.fetchAll('UPDATE bank_cards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                ...
                if self.source ~= -1 then
                    ...
                    local player = QRCore.Functions.GetPlayer(self.source)
...
RegisterServerEvent('qr-banking:server:registerCurrentAccount')
AddEventHandler('qr-banking:server:registerCurrentAccount', function(cid)
...
RegisterServerEvent('qr-banking:server:registerSavingsAccount')
AddEventHandler('qr-banking:server:registerSavingsAccount', function(cid)
