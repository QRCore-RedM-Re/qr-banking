...
RegisterServerEvent('qr-banking:server:modifyBank')
AddEventHandler('qr-banking:server:modifyBank', function(bank, k, v)
...
RegisterServerEvent('qr-base:itemUsed')
AddEventHandler('qr-base:itemUsed', function(_src, data)
    if data.item == "moneybag" then
        TriggerClientEvent('qr-banking:client:usedMoneyBag', _src, data)
    end
end)

RegisterServerEvent('qr-banking:server:unpackMoneyBag')
...
AddEventHandler('qr-banking:doQuickDeposit', function(amount)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            ...
            TriggerEvent('qr-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash deposit of $"..amount.." successfully.")
        end
    end
end)
...
AddEventHandler('qr-banking:doQuickWithdraw', function(amount, branch)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then
            ...
            TriggerEvent('qr-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash withdrawal of $"..amount.." successfully.")
        end
    end
end)
...
AddEventHandler('qr-banking:savingsDeposit', function(amount)
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerEvent('qr-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings deposit of $"..tostring(amount).." successfully..")
    end
end)
...
AddEventHandler('qr-banking:savingsWithdraw', function(amount)
    ...
    if tonumber(amount) <= currentSavings then
        ...
        TriggerEvent('qr-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings withdrawal of $"..tostring(amount).." successfully.")
    end
end)
...
AddEventHandler('qr-banking:createSavingsAccount', function()
    ...
    TriggerEvent('qr-log:server:CreateLog', 'banking', 'Banking', "lightgreen", "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** opened a savings account")
end)
				...
				if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(id))) < distance then
					if xPlayer.Functions.RemoveMoney('cash', amount) then
						if xReciv.Functions.AddMoney('cash', amount) then
							...
							TriggerClientEvent('QRCore:Notify', src, 9, "Could not give item to the given id.", 2000, 0, 'mp_lobby_textures', 'cross')
						end
					else
						TriggerClientEvent('QRCore:Notify', src, 9, "You don\'t have this amount.", 2000, 0, 'mp_lobby_textures', 'cross')
					end
				else
					TriggerClientEvent('QRCore:Notify', src, 9, "You are too far away lmfao.", 2000, 0, 'mp_lobby_textures', 'cross')
				end
			else
				TriggerClientEvent('QRCore:Notify', src, 9, "You are dead LOL.", 2000, 0, 'mp_lobby_textures', 'cross')
			end
		else
			TriggerClientEvent('QRCore:Notify', src, 9, "Wrong ID.", 2000, 0, 'mp_lobby_textures', 'cross')
		end
	else
		TriggerClientEvent('QRCore:Notify', src, 9, "Usage /givecash [ID] [AMOUNT]", 2000, 0, 'mp_lobby_textures', 'cross')
	end
end)
